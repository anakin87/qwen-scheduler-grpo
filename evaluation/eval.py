...
def evaluate(results_path):
    ...
    format_errors, less_than_2_events, overlaps, non_sorted, non_existing, valid_schedules = 0, 0, 0, 0, 0, 0
    ...
    for i in range(0, 100):
        ...
        existing_events = {ev for ev in events if [ev[0], ev[1], ev[2]] in original_example["events"]}
        if len(existing_events) < len(events):
            scores.append(0)
            non_existing += 1
            continue

        events_minutes = [(ev[0], time_to_minutes(ev[1]), time_to_minutes(ev[2])) 
                            for ev in events]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Evaluate results with configurable path')
